...
# 5:  Multilayout value
# 7:  Multilayout name/label (needs to be in at least one of the keys for any given multilayout option. 
#     If there is a list of multilayout options, at least one key of each value should have a label.)
# 9:  Row
# 11: Col
...
write_file(serialized_path, json.dumps(srlzd, ensure_ascii=False, indent=2))
...
via_product_id = '0x1111'
via_vendor_id = '0x1111'
via_json_path = 'via.json'
via_json_content = kbd_to_via(keyboard, via_product_id, via_vendor_id)
write_file(via_json_path, json.dumps(via_json_content, ensure_ascii=False, indent=2))
...
vial_product_id = None
vial_vendor_id = None
vial_uid = gen_uid()
vial_json_path = 'vial.json'
vial_config_h_path = 'config.h'
vial_json_content, vial_config_h = kbd_to_vial(keyboard, vial_uid, vial_product_id, vial_vendor_id)
write_file(vial_json_path, json.dumps(vial_json_content, ensure_ascii=False, indent=2))
